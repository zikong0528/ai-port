...
src/main/
  ...
  ipc.js             全部 IPC：扫描合并/列表/状态/启停/实例/图标/logo/设置/诊断/关于/更新/导入导出/撤销
...
- **自测清单**：核心（扫描/图标/启动/终止/多开/窗口自动关闭/窗口标题时间）；细节（备注/重命名/置顶/撤销/深色/语言/导入导出/关于/退出无残留/窗口位置记忆）；边界（改名后仍识别/断网正常）。
