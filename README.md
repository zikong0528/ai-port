...
一个类似游戏启动器的免费 AI 工具：把设备上的 AI 应用与命令行 agent 统一收进一个列表，一键启动、一键终止。

对于 `claude` / `dsh` / `codex` 这类命令行 agent，以往要手动打开终端、敲命令才能唤醒；AI Port 把它们也收进列表，点一下就在终端里运行起来，免去进命令行的麻烦。

同时也能统一管理聊天客户端、本地大模型运行时、代码编辑器等桌面应用。
